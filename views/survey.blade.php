...
@section('content')
	...
	<form action="/submit" method="POST">
		...
		@if($submitted)
			<div class="notice">You have already submitted a survey. You may submit again to update your previous submission.</div>
		@endif
		...
		<div class="input"><input type="text" name="student_id" value="{{{$dflt->get('student_id')}}}" /></div>
		--}}

		<input type="hidden" name="student_id" value="0" />

		<div class="label">Net ID: (<a href="{{{ URL::to('AuthController@logout') }}}">Not You?</a>)</div>
		...
		<div class="input"><input type="text" name="first_name" value="{{{$dflt->get('first_name')}}}" /></div>

		<div class="label">Last Name:</div>
		{{berror($errors,'last_name')}}
		<div class="input"><input type="text" name="last_name" value="{{{$dflt->get('last_name')}}}" /></div>

		<div class="label">Email Address:</div>
		{{berror($errors,'email_address')}}
		<div class="input"><input type="text" name="email_address" value="{{{$dflt->get('email_address')}}}" /></div>
		...
		<div class="input"><select name="college">
			...
			@foreach($colleges as $cid => $cname)
				<option value="{{{$cid}}}" {{$dflt->selected('college',$cid)}}>
					{{$cname}}
				</option>
			@endforeach
		</select></div>
		...
		<div class="input"><select name="year">
			...
			@foreach($years as $cid => $cname)
				<option value="{{{$cid}}}" {{$dflt->selected('year',$cid)}}>
					{{$cname}}
				</option>
			@endforeach
		</select></div>
		...
		<div class="input"><select name="major">
			...
			@foreach($majors as $cid => $cname)
				<option value="{{{$cid}}}" {{$dflt->selected('major',$cid)}}>
					{{$cname}}
				</option>
			@endforeach
		</select></div>
		...
		<div class="input"><select name="gender">
			...
			@foreach($genders as $gid => $gname)
				<option value="{{{$gid}}}"  {{$dflt->selected('gender',$gid)}}>
					{{{$gname}}}
				</option>
			@endforeach
		</select></div>
		...
		<div class="text">You will be matched only with people of the gender(s) you indicate who also express an interest in your gender.</div>
		@foreach($genders as $gid => $gname)
			<div class="input"><label>
				<input type="checkbox" name="interested_{{{$gid}}}" value="1"  {{$dflt->checked('interested_'.$gid, '1')}} /> {{{$gname}}}
			</label></div>
		@endforeach
		...
		@foreach($questions as $qid => $qdata)
			...
			@foreach($qdata['options'] as $oid => $otext)
				<div class="input"><label>
					<input type="radio" name="question_{{{$qid}}}" value="{{{$oid}}}" {{$dflt->checked('question_'.$qid,$oid)}} />
					{{{ $otext }}}
				</label></div>
			@endforeach
		@endforeach
		...
		<div class="input"><label>
			<input type="checkbox" name="send_results" value="1" {{$dflt->checked('send_results', '1')}} />
